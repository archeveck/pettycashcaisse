...
async function debug() {
  try {
    ...
    const fields = await odooCall(config, 'object', 'execute_kw', [
      ...
      'account.analytic.account', 'fields_get', [], { attributes: ['string', 'type', 'relation'] }
    ])
    console.log('Analytical Account Fields:', Object.keys(fields).filter(f => f.includes('project')))

    console.log('Fetching fields for project.project...')
    const pFields = await odooCall(config, 'object', 'execute_kw', [
      config.db, uid, config.password,
      'project.project', 'fields_get', [], { attributes: ['string', 'type', 'relation'] }
    ])
    console.log('Project Fields:', Object.keys(pFields).filter(f => f.includes('analytic')))
